...
const DUCK_PARTS = DUCK_BODY.concat(DUCK_PAT, DUCK_HEAD);
...
/* Where a part sits is worked out from the anchors every time it is
   ...
   every hat, band and fringe moves with it, which is the whole reason
   the anchors exist; the test moves the head and checks that they did.
   ...
   fringe in the catalogue ends there or higher. */
function duckFit(){
  const H = DUCK.HEAD, B = DUCK.BODY;
  ...
  return {H, B, n: svgn, at, wAt, band, dome,
...
function duckSVG(it, outfit){
  ...
  // the layers still to come; the body, the pattern and the head are
  // the three that are already here
  const fit = duckFit();
  const pat = patPart ? `<g clip-path="url(#duckskin)">${patPart.draw(fit, rim)}</g>` : "";
  const headwear = headPart ? headPart.draw(fit, rim) : "";
  const back = "", eyewear = "", front = "";
  const g = defs + back
    + edge
    + shell(c2, skin, "")
    + pat
...
/* The three layers in the order the garage shows them: the body first,
   because it is the one that is never empty, then what is painted on
   and what goes on the head. Cheapest part first inside each of them,
   so the first tile a child sees is one they can afford. */
const DUCK_LAYERS = [
