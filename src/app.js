...
/* A track that has once been open never closes again. Adding material
   ...
   Older profiles are seeded from where they have demonstrably been:
   a finished race there, or a single fact of that track in the box. */
function seedOpened(p){
  p.opened = p.opened || {};
  for(const tr of TRACKS){
    if(p.opened[tr.id]) continue;
    const raced = ((p.trackRuns || {})[tr.id] || 0) > 0;
    if(raced || trackKeys(p, tr).some(k => (p.facts[k] || {}).reps > 0)) p.opened[tr.id] = true;
  }
}
...
function viewResult(p){
  ...
  const weak = [...new Map(RUN.wrongKeys.map(i => [i.text, i])).values()].slice(0, 6);
  const weakHtml = weak.length ? `
    <div class="h2" style="margin-bottom:6px">${t("reviewNext")}</div>
    <div class="factchips">${weak.map(i => `<span class="factchip">${i.text} = ${i.answer}</span>`).join("")}</div>` : "";
