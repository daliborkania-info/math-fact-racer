...
/* --- the duck ---
   ...
   The draw order below has empty slots in it. They are the layers, and
   today every one of them is empty: this duck wears nothing at all. */
const DUCK = {
...
function duckSVG(it){
  ...
  // the layers, all empty today: nothing is worn and nothing is for sale
  const back = "", pat = "", eyewear = "", headwear = "", front = "";
  const g = back
    // the tail goes in before the body, so the body hides where it grows out
    + `<path d="M${T.x+4} ${T.y-4} L${T.x-22} ${T.y-15} L${T.x-2} ${T.y+14} Z" fill="${it.c2}"/>`
    + `<ellipse cx="${B.x}" cy="${B.y}" rx="${B.rx}" ry="${B.ry}" fill="${it.c1}"/>`
    // the head overlaps the body, so the neck is the notch the two leave
    + `<circle cx="${H.x}" cy="${H.y}" r="${H.r}" fill="${it.c1}"/>`
    + pat
    ...
    + ` Q ${W.x-2} ${W.y+12} ${W.x-14} ${W.y+6} Z" fill="${it.c2}"/>`
    + `<path d="M${K.x} ${K.y-5} Q ${K.x+14} ${K.y-6} ${K.x+17} ${K.y-1}`
    + ` Q ${K.x+18} ${K.y+2} ${K.x+13} ${K.y+4} Q ${K.x+6} ${K.y+6} ${K.x} ${K.y+5} Z"`
    + ` fill="#ff9f1c"/>`
