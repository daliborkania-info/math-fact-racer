...
// V hlavnim bloku je tedy 16 trati: t1 az t5, d1, a100, clock, sest trati
// tretiho rocniku a k tomu sampionat se slabymi misty. Plus dilna a dvere
// zpatky je to 18 mist; cest je 16, dilna ani dvere nahled nemaji.
// Sest trati tretiho rocniku je od kroku D2: retezec, poradi operaci,
// za nasobilkou, kulata cisla, zaokrouhlovani a tisicovka.
const cesty=()=>new Set(qa('.thumb svg path').map(p=>p.getAttribute('d'))).size;
ok('slozena mapa treťáka ma 16 ruznych cest', cesty()===16, cesty()+' okruhu');
ok('slozena mapa ma 18 mist', qa('.world .place').length===18, qa('.world .place').length+' mist');
...
ok('rozbaleni ukazalo minule roky', cesty()===23, cesty()+' okruhu');
...
// rozbaleno je to porad 25 mist, jen jich vic patri do hlavniho bloku:
// 1 dvere + 7 minulych + 16 letosnich + dilna
ok('mista lezi podel cesty, ne pod sebou', qa('.world .place').length===25 && qa('.worldroad path').length>0,
...
ok('kapitoly bez generatoru jsou nevybratelne', zamcene().length===11, zamcene().length+' zamcenych z 33');
...
click(q('[data-act="back"]'));
ok('rozbalena mapa treťáka je delsi', qa('.place:not(.peekdoor)').length===25 && /Do tří/.test(txt()),
...
ok('prepnuti hrace mapu slozilo', qa('.place.past').length===0 && qa('.place.backdoor').length===1
   && qa('.place:not(.peekdoor)').length===18);
...
ok('ctvrtak vidi celou mapu jako driv',
   qa('.place').length===24 && /Do tří/.test(txt()) && /Hodiny/.test(txt()) && /Do tisíce/.test(txt()),
